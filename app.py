...
# OVDE KOPIRAJ CELI master_strings REČNIK IZ multi-jezik5a.py
...
# OVDE KOPIRAJ main_categories_translations, subcategories_translations, 
# i product_parts_translations IZ multi-jezik5a.py
...
def stranica_zalihe():
    ...
    st.title("📊 Stanje zaliha")
    
    # Pretraga
    search_term = st.text_input("🔍 Pretraži proizvode")
    ...
    if not df.empty:
        ...
        if len(df) > 0:
            ...
            if selected_product:
                ...
                with st.expander("Ažuriraj proizvod", expanded=False):
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        new_name = st.text_input("Naziv", value=product_data['product_name'])
                        new_desc = st.text_input("Opis", value=product_data['description'])
                        new_piece = st.text_input("Komad", value=product_data['piece'])
                    ...
                    with col_save:
                        if st.button("💾 Sačuvaj promene", use_container_width=True):
                            update_product_in_db(
                                product_id, new_name, new_desc, new_piece, 
                                new_quantity, new_unit
                            )
                            st.success("✅ Proizvod ažuriran!")
                            st.rerun()
                    
                    with col_delete:
                        if st.button("🗑️ Obriši", use_container_width=True, type="secondary"):
                            if st.checkbox("Potvrdite brisanje"):
                                delete_product_from_db(product_id)
                                st.success("✅ Proizvod obrisan!")
                                st.rerun()
        else:
            st.info("Nema proizvoda za ažuriranje")
    else:
        st.info("ℹ️ Nema proizvoda u zalihama")
        
    # Dugme za nazad
    if st.button("⬅️ Nazad na glavni meni"):
        st.session_state.korak = "kategorije"
        st.rerun()
...
def stranica_spisak():
    ...
    with col1:
        if st.button("📧 Pošalji email", use_container_width=True):
    ...
    with col3:
        if st.button("📋 Kopiraj", use_container_width=True):
            ...
            st.success("✅ Spisak kopiran u clipboard!")
    
    with col4:
        if st.button("🗑️ Obriši sve", use_container_width=True, type="secondary"):
            if st.checkbox("Potvrdite brisanje svih stavki"):
                clear_shopping_list()
                st.success("✅ Spisak obrisan!")
                st.rerun()
    ...
    if not df.empty:
        ...
        st.subheader("Brisanje stavki")
        items_to_delete = st.multiselect(
            "Izaberite stavke za brisanje",
            df['product_name'].tolist()
        )
        
        if items_to_delete and st.button("Obriši označene", type="primary"):
            delete_items_from_list(items_to_delete)
            st.success(f"✅ Obrisano {len(items_to_delete)} stavki!")
            st.rerun()
    else:
        st.info("ℹ️ Spisak je prazan")
...
def send_to_messenger(df):
    ...
    st.success("✅ Spisak je pripremljen! Kopirajte tekst iznad i nalepite u Messenger.")
...
def copy_to_clipboard(df):
    ...
    # Prikaži poruku za kopiranje
    st.code(message, language=None)
    st.success("✅ Spisak je pripremljen za kopiranje!")
...
with st.sidebar.expander("🔧 Debug Info", expanded=False):
    ...
    st.write(f"Kategorija: {st.session_state.get('trenutna_kategorija', 'N/A')}")
    st.write(f"Podkategorija: {st.session_state.get('trenutna_podkategorija', 'N/A')}")
    st.write(f"Deo proizvoda: {st.session_state.get('trenutni_deo_proizvoda', 'N/A')}")
    
    # Direktni linkovi za test
    st.markdown("---")
    st.write("**Direktni linkovi:**")
    if st.button("Test → Unos stranica"):
